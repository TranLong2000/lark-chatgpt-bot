...
async function fetchWOWBUY() {
  try {
    ...
    // 8️⃣ Kết nối socket.io (cURL 13)
    await axios.get('https://report.wowbuy.ai:39888/socket.io/?token=' + authToken + '&EIO=3&transport=polling&t=' + Date.now() + '-1', {
      headers: {
        'sec-ch-ua-platform': '"Windows"',
    ...
    // 9️⃣ Kết nối socket.io bổ sung (cURL 14)
    await axios.get('https://report.wowbuy.ai:38889/socket.io/?token=' + authToken + '&EIO=3&transport=polling&t=' + Date.now() + '-2', {
      headers: {
        'sec-ch-ua-platform': '"Windows"',
        'Referer': 'https://report.wowbuy.ai/',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36',
    ...
    // 🔟 Lấy tài nguyên JavaScript (cURL 15)
    ...
    // 1️⃣1️⃣ Truy vấn tham số yêu thích (cURL 16)
    ...
    // 1️⃣2️⃣ Truy vấn tham số dialog (cURL 17)
    ...
    // 1️⃣3️⃣ Thu thập thông tin preview (cURL 18)
    ...
    // 1️⃣4️⃣ Lấy báo cáo và trích xuất dữ liệu (cURL 19)
    ...
    // 1️⃣5️⃣ Trích xuất dữ liệu từ bảng HTML
